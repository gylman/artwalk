...
export async function POST() {
  await db.delete(walks).execute();
  await db.delete(challenges).execute();
  await db.delete(nfts).execute();

  let random = cs.find((x) => x.id === "square")!;
  const ids: string[] = [];

  for (let i = 0; i < 4; i++) {
    while (!random || ids.includes(random.id)) {
      random = cs[Math.floor(cs.length * Math.random())];
    }
  ...
  for (let i = 0; i < 4; i++) {
    ...
    await db
      .insert(challenges)
      .values({
        title: `${["Creative", "Wonderful"][Math.floor(Math.random() * 2)]} ${
          random.title
        }${Math.random() < 0.5 ? " Challenge" : ""}`,
        imageUrl: random.imgUrl,
        type: "creative",
